...
<body>
	<h1>JSON?</h1>
	<p>
		현재는 xml 보다는 json 으로 데이터를 주고 받는 경우가 많다.
		JSON (Java Script Object Notation) 은, xml 보다 더 사람이 읽고 쓰기 용이하고 ,
		기계가 분석하고 만들어내기도 휠씬 쉬운 형태의 데이터 표기법이다. 
	</p>
	<p>
		Javascript에서 사용하는 데이터표기법이 있는데 , 이 형태의 문자열이라면
		바로 객체를 만들어내는게 가능하기 때문에 , 분석하기가 쉽다.
	</p>
	<o>
		수치 , 논리  = 값만 작성 /
					 문자열  : 홀따옴표 or 쌍따움표로 가능하지만 JSON 방식은 쌍따옴표만 가능함.
		배열 = [ ] , 객체 : {변수명:값 ,변수명:값,,,} 
	</o>
